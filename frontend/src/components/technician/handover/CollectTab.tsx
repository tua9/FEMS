...
function toDetailRecord(loan: ActiveLoan): HandoverDetailRecord {
  const statusClass =
...
const MarkReturnedModal: React.FC<MarkReturnedModalProps> = ({ loan, onClose, onConfirm }) => {
  const initials = loan.borrower.name
    .split(' ').slice(0, 2).map((w) => w[0]).join('').toUpperCase();
      ...
      >
        ...
        <div className="px-7 pt-7 pb-5 flex items-start justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-emerald-100 flex items-center justify-center">
              <span className="material-symbols-outlined text-emerald-600 text-2xl">assignment_turned_in</span>
            </div>
            <div>
              <h2 className="text-base font-extrabold text-slate-800">Mark as Returned</h2>
              <p className="text-xs text-slate-500 mt-0.5">Confirm equipment return for this loan</p>
            </div>
          </div>
        ...
        <div className="px-7 py-6 space-y-5">
          ...
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-200">
            ...
            <div>
              <p className="text-sm font-bold text-slate-800">{loan.borrower.name}</p>
              <p className="text-[11px] text-slate-500">{loan.borrower.idLabel}</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
            ...
            <p className="text-[11px] text-slate-500 pl-6">{loan.itemNames}</p>
          </div>
          ...
          <div className="flex items-center gap-2 px-1">
            ...
            <p className={`text-xs font-semibold ${
              loan.dueColor === 'red' ? 'text-red-500' :
              loan.dueColor === 'yellow' ? 'text-yellow-600' : 'text-slate-600'
            }`}>{loan.dueLabel}</p>
          </div>
...
interface CardProps { loan: ActiveLoan; onDetails: () => void; onMarkReturned: () => void }

const LoanCard: React.FC<CardProps> = ({ loan, onDetails, onMarkReturned }) => {
  const st = getLoanStatusStyle(loan.status);

  const dueTextColor =
    loan.dueColor === 'red'    ? 'text-red-500'    :
    loan.dueColor === 'yellow' ? 'text-yellow-600' : 'text-slate-700';
  ...
  return (
    ...
    >
      <div className="p-6 flex-1">
        ...
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-3">
            {loan.borrower.avatar ? (
              <img src={loan.borrower.avatar} alt={loan.borrower.name} className="w-10 h-10 rounded-full border border-slate-200 object-cover" />
            ) : (
              <div className="w-10 h-10 rounded-full bg-[#1A2B56]/10 flex items-center justify-center font-bold text-[#1A2B56] text-xs border border-slate-200">
                {initials}
              </div>
      ...
      <div className="p-4 bg-slate-50/50 border-t border-slate-200 flex gap-2">
        <button
          onClick={onDetails}
          className="flex-1 bg-white hover:bg-slate-100 text-slate-600 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 border border-slate-200"
        >
        ...
        <button
          ...
          className="flex-1 bg-white hover:bg-emerald-600 hover:text-white text-emerald-700 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 border border-emerald-200"
        >
...
const CollectTab: React.FC = () => {
  const [loans, setLoans] = useState<ActiveLoan[]>([...MOCK_ACTIVE_LOANS]);
