...
export interface ReportEntry {
    ...
    subject: string;
    date: string;
    status: 'pending' | 'approved' | 'rejected' | 'processing' | 'fixed';
...
export const RecentReports: React.FC<RecentReportsProps> = ({ newReport }) => {
    ...
    const fetchedReports: ReportEntry[] = myReports.map(r => {
        ...
        if (r.type === 'equipment' && r.equipment_id && typeof r.equipment_id !== 'string') {
           ...
           subjectStr = r.description.substring(0, 30) + (r.description.length > 30 ? '...' : '');
        }
    ...
    const displayReports: ReportEntry[] = newReport
    ...
    return (
        <div className="mt-[3rem] dashboard-card p-[2rem] lg:p-[2.5rem] rounded-[2.5rem]">
            ...
            <div className="flex items-center justify-between mb-[2rem]">
                ...
                <button
                    onClick={() => navigate('/student/borrow-history?tab=report')}
            ...
            <div className="space-y-[0.75rem]">
                {loading && fetchedReports.length === 0 ? (
                   <div className="text-center py-4 text-slate-500 text-sm">Loading recent reports...</div>
                ) : displayReports.length === 0 ? (
                   <div className="text-center py-4 text-slate-500 text-sm">No recent reports found.</div>
                ) : (
                    displayReports.map((report) => (
                        <div
                            key={report.id}
                            onClick={() => report.originalReport ? setModal({ type: 'report', item: report.originalReport }) : navigate('/student/borrow-history?tab=report')}
                        ...
                        >
                            ...
                            <div className="md:col-span-3">
                                ...
                                <p className="text-[0.875rem] font-bold text-[#1E2B58] dark:text-white group-hover:underline max-w-xs truncate" title={report.id}>{report.id.substring(0, 8)}...</p>
                            </div>
                            {/* Subject */}
