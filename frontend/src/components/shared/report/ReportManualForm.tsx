...
export const ReportManualForm: React.FC<ReportManualFormProps> = ({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof ReportFormData | "files" | "roomId", string>>
  >({});
  ...
  useEffect(() => {
    if (prefillRoomId) {
      ...
      // Auto-select building if a room is prefilled
      const room = rooms.find(r => r._id === prefillRoomId);
      if (room && room.building_id) {
        setSelectedBuildingId(
          typeof room.building_id === 'string'
            ? room.building_id
            : room.building_id._id
        );
      }
    }
  }, [prefillRoomId, rooms]);
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-[1rem] sm:grid-cols-3 md:grid-cols-5">
          {CATEGORIES.map((cat) => {
            const Icon = cat.icon;
            ...
            return (
              <label key={cat.id} className="group cursor-pointer">
                ...
                >
                  <Icon
                    className={`h-[2rem] w-[2rem] ${isSelected
                  ...
                  />
                  <span
                    className={`text-[0.625rem] font-bold tracking-wider uppercase ${isSelected
                        ? "text-[#1E2B58] dark:text-white"
                        : "text-slate-500 dark:text-slate-400"
                      }`}
                  >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-[1rem]">
          ...
          <div className="relative">
            <div className="pointer-events-none absolute inset-y-0 left-[1.25rem] flex items-center">
              <MapPin
                className={`h-[1.25rem] w-[1.25rem] ${errors.roomId && !selectedBuildingId ? "text-red-400" : "text-slate-400"
                  }`}
              />
            </div>
            <select
            ...
            >
              <option value="">Select Building ({buildings.length} / {rooms.length})</option>
              {buildings.map((b) => (
          ...
          <div className="relative">
            ...
            >
              <option value="">
                {selectedBuildingId ? "Select Room" : "Select Building First"}
              </option>
        ...
        {errors.roomId && (
          <p className="mt-2 pl-1 text-xs font-bold text-red-500 dark:text-red-400">
            {errors.roomId}
          </p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="mt-1.5 flex justify-between px-1">
          ...
          <span
            className={`text-[0.625rem] font-bold ${description.length < 10 ? "text-slate-400" : "text-emerald-500"
              }`}
          >
      ...
      <div>
        <h3 className="mb-[1rem] text-[0.6875rem] font-bold tracking-[0.15em] text-slate-500 uppercase dark:text-slate-400">
          4. Upload Evidence{" "}
          <span className="font-medium normal-case opacity-60">
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="mt-[0.25rem] text-[0.625rem] tracking-wider text-slate-400 uppercase dark:text-slate-500">
          ...
          >
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="hover:scale-110 active:scale-95 rounded-xl bg-white/40 p-[0.5rem] text-slate-600 transition-all hover:bg-white/60 dark:bg-white/10 dark:text-slate-300 dark:hover:bg-white/20"
            ...
            {files.map((file, i) => (
              <div key={i} className="group relative">
                <img
                  ...
                  alt={file.name}
                  className="h-16 w-16 rounded-xl border border-white/60 object-cover shadow-sm dark:border-white/10"
                />
                <button
                  type="button"
                  onClick={() => removeFile(i)}
                  className="hover:scale-110 active:scale-90 absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white opacity-0 shadow-sm transition-all group-hover:opacity-100"
                >
