...
const STATUS_LABEL= {
 good: 'Tốt', in_use: 'Đang mượn', maintenance: 'Bảo trì', broken: 'Hỏng', reserved: 'Đặt trước',
}
...
const MaintenanceAttentionList = ({ items }) => {
 ...
 <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 ${STATUS_COLOR[item.status] ?? 'bg-slate-100 text-slate-600'}`}>
 {STATUS_LABEL[item.status] ?? item.status}
 </span>
 </div>
 <div className="flex gap-3 flex-shrink-0 ml-2 text-[10px] font-semibold text-slate-500 dark:text-slate-400">
