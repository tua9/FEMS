          ...
          <div className="dashboard-card flex h-full flex-col rounded-4xl p-8">
            ...
            <div className="flex-1 space-y-3">
              {UPCOMING_CLASSES.map((session, index) => (
                <button key={session.id} onClick={() => navigate("/lecturer/calendar")} className={`dashboard-card group w-full rounded-2xl p-4 text-left transition-all hover:shadow-md ${index === UPCOMING_CLASSES.length - 1 ? "opacity-60" : ""}`}>
