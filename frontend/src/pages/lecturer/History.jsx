...
const History = () => {
 ...
 {(['report', 'borrow', 'approval']).map((tab) => (
 <button
 key={tab}
 ...
 <span className="font-bold">{(req.equipment_id)?.name || (req.room_id)?.name || 'Unknown Item'}</span>
 <span className="text-[10px] opacity-60">{(req.equipment_id)?.category || (req.room_id)?.type || 'Equipment'}</span>
 </div>
 </td>
