...
export const ApprovalCenter: React.FC = () => {
    ...
    const mappedRequests = useMemo<BorrowRequest[]>(() => {
        return pendingBorrowRequests.map((r: any) => ({
            id: r._id,
            status: r.status as RequestStatus,
            student: {
                name: r.user_id?.displayName || r.user_id?.username || 'Unknown Student',
                id: (r.user_id?._id || 'UID').substring(0, 8),
                initials: (r.user_id?.displayName || r.user_id?.username || 'US').substring(0, 2).toUpperCase(),
                statusColor: 'bg-green-500',
                avatar: r.user_id?.avatarUrl
            },
            equipment: {
                name: r.equipment_id?.name || r.room_id?.name || 'Unknown Equipment',
                asset: (r.equipment_id?._id || r.room_id?._id || 'ASSET').substring(0, 8),
                icon: Laptop,
                type: 'laptop'
            },
            period: {
                ...
                labelColor: 'text-[#1E2B58] dark:text-blue-300'
            },
        }));
    }, [pendingBorrowRequests]);

    const requests = mappedRequests.length > 0 ? mappedRequests : INITIAL_REQUESTS;
    ...
    const confirmApprove = async () => {
        ...
        try {
            await borrowRequestService.approveBorrowRequest(approvingReq.id);
            fetchPendingBorrowRequests();
            showToast('success', `Approved: ${approvingReq.student.name}'s request for ${approvingReq.equipment.name}.`);
        } catch (error) {
            showToast('error', `Failed to approve request.`);
            // Mock fallback if id length is small (it's a fake item from INITIAL_REQUESTS)
            if (approvingReq.id.length < 5) {
                showToast('info', `Mock Approved: ${approvingReq.student.name}'s request.`);
            }
        }
    ...
    const confirmReject = (e: React.FormEvent) => {
        ...
        // This is still local mock since we don't have reject endpoint yet.
