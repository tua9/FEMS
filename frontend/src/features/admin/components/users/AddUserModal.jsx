...
const AddUserModal = ({ isOpen, onClose, user, onUserUpdated }) => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setEmail(user.email);
      setRole(user.role);
      setClassId(user.classId?._id || user.classId || '');
    ...
    try {
      if (avatarFile) {
        ...
        if (uploadRes.data?.files?.length > 0) {
          finalAvatarUrl = uploadRes.data.files[0].url;
          finalAvatarId = uploadRes.data.files[0].filename;
        }
      }
      ...
      const userData = {
        ...
        ...(password ? { password } : {}),
        ...(role === 'student' ? { classId: classId || null } : {}),
      };
      ...
      toast.error(error?.response?.data?.message || error.message || "Failed to save user records.");
    }
  };
        ...
        <div className="p-10 pt-0 overflow-y-auto no-scrollbar space-y-8 relative z-10 mt-6">
          <form id="userForm" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-1.5 pt-2">
              ...
              <div className="border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl p-8 flex flex-col items-center justify-center text-center bg-slate-50/30 dark:bg-black/10 hover:bg-slate-50/50 transition-colors cursor-pointer group relative overflow-hidden">
                <input
                  type="file"
                  accept="image/*"
                ...
                />
                {avatarPreview ? (
                  <img src={avatarPreview.startsWith('http') || avatarPreview.startsWith('blob') ? avatarPreview : `http://localhost:5001${avatarPreview}`} alt="Preview" className="w-24 h-24 rounded-full object-cover shadow-md mb-3 border-4 border-white dark:border-slate-800" />
                ) : (
                ...
                <p className="text-xs font-black text-slate-700 dark:text-slate-300">{avatarPreview ? 'Change Avatar' : 'Click to upload avatar'}</p>
