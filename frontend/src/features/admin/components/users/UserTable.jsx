            ...
            <table className="w-full text-left border-separate border-spacing-y-3 table-fixed">
                ...
                <thead>
                    <tr className="text-slate-800 dark:text-slate-300">
                        ...
                        <th className="px-4 pb-2 text-[10px] font-semibold uppercase tracking-[0.15em] opacity-80">Class</th>
                ...
                <tbody>
                    {users.length > 0 ? (
                        users.map(user => (
                            <tr key={user._id} className="group cursor-pointer" onClick={() => onOpenDetails && onOpenDetails(user)}>
                                ...
                                <td className={`p-3 ${rowBg}`}>
                                    {user.role === 'student' ? (
                                        user.classId ? (
                                            <span className="text-xs font-bold text-[#1A2B56] dark:text-blue-400">{user.classId?.code || 'Assigned'}</span>
                                        ) : (
                                            <span className="text-xs font-medium italic text-slate-400 dark:text-slate-500">Unadded</span>
                                        )
                                    ) : (
                        ...
                        <tr>
                            <td colSpan={6} className="py-12 text-center bg-white/40 dark:bg-slate-800/40 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
