...
const BorrowingManagement = () => {
  ...
  const sortedRecords = borrowRecords
    .filter(record => {
      ...
      const isRecOverdue = record.status === 'overdue' || (record.status === BORROW_STATUS.HANDED_OVER && new Date(record.expectedReturnDate) < new Date());
  ...
  const isOverdueGlobal = (r) =>
    r.status === 'overdue' || (r.status === BORROW_STATUS.HANDED_OVER && new Date(r.expectedReturnDate) < now);
  ...
  const isBlurred = isDetailModalOpen;
      ...
      <div className={`transition-all duration-300 ${isBlurred ? 'filter blur-sm opacity-50 pointer-events-none' : ''}`}>
        <div className="mb-8 px-2 flex flex-col md:flex-row md:items-center justify-between gap-6 mt-2">
          <PageHeader
            ...
            subtitle="Manage equipment loans, approvals, and active monitoring."
