...
const RemindModal = ({ isOpen, onClose, request, onConfirm, submitting }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setNote(`Vui lòng hoàn trả thiết bị ${request?.code || ''} sớm nhất có thể.`);
    } else {
      setNote('');
    }
  }, [isOpen, request]);
    ...
    >
      <div className="dashboard-card rounded-4xl p-8 w-full max-w-md shadow-2xl shadow-[#1E2B58]/20 relative animate-in fade-in zoom-in-95 duration-200 bg-white dark:bg-[#0B1120]">
        ...
        <div className="flex flex-col items-center text-center mt-2 mb-6">
          ...
          <h2 className="text-xl font-black text-[#1E2B58] dark:text-white">Gửi lời nhắc</h2>
          <p className="text-sm font-bold text-slate-500 mt-2">
            Gửi nhắc nhở mượn quá hạn tới <span className="text-[#1E2B58] dark:text-white font-black">{getStudentName(request)}</span>.
          </p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-amber-50/50 dark:bg-amber-900/10 border border-amber-100/50 dark:border-amber-900/20 mb-6 flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-black text-amber-700 dark:text-amber-400">Thiết bị cần thu hồi</p>
