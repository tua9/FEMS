    ...
    >
      <div className="dashboard-card rounded-4xl p-8 w-full max-w-md shadow-2xl shadow-[#1E2B58]/20 relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="space-y-0 text-sm">
          {[
            ['Thiết bị', request.equipmentId?.name || '—'],
            ['Trạng thái', <BorrowBadge key="s" status={request.status} req={request} />],
            ['Lý do mượn', request.note || '—'],
            ['Gửi lúc', fmtDateTime(request.createdAt)],
            ['Phản hồi', request.decisionNote || '—'],
          ].map(([label, value]) => (
            <div key={label} className="flex justify-between items-center gap-4 py-3 border-b border-[#1E2B58]/5 dark:border-white/5 last:border-0">
