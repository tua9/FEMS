...
import { X, LogOut, Package, CheckCircle2, Loader2 } from 'lucide-react';
...
const ReturnModal = ({ isOpen, onClose, target, onConfirm, submitting }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (returnFiles.length === 0) {
      toast.error('Please provide at least one photo of the equipment being returned.');
      return;
    }
    onConfirm({
      checklist: returnChecklist,
      notes: returnNote.trim() || null,
      files: returnFiles, // Pass raw files to the page handler
    });
  };
    ...
    >
      <div className="dashboard-card rounded-4xl p-8 w-full max-w-md shadow-2xl shadow-[#1E2B58]/20 relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-white/40 dark:bg-slate-800/40 rounded-[1.25rem] p-4 mb-4 space-y-2.5">
          <p className="text-[0.625rem] font-black uppercase tracking-widest text-[#1E2B58]/50 dark:text-white/40 mb-2">Condition Checklist</p>
          {[
            { key: 'appearance',  label: 'Good physical condition (no scratches or cracks)' },
            ...
            <label key={key} className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                className="w-4 h-4 rounded accent-[#1E2B58] cursor-pointer"
              />
        ...
        <div className="flex flex-col gap-2 mb-4">
          ...
          <textarea
            rows={2}
            placeholder="Describe the equipment's condition when returning (scratches, missing accessories, etc.)..."
            value={returnNote}
        ...
        <div className="mb-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {returnPreviews.map((src, idx) => (
              <div key={idx} className="relative group aspect-video rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 shadow-sm">
