...
const BorrowBadge = ({ status, req }) => {
  ...
  const approvedLabel = req?.handoverInfo?.submittedAt ? 'Awaiting Receipt' : 'Approved';
  const cfg = {
    ...
    handed_over: { label: 'In Use',              cls: 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-900/30' },
    returning:   { label: 'Awaiting Return',     cls: 'bg-violet-50 text-violet-600 border-violet-200 dark:bg-violet-900/20 dark:text-violet-400 dark:border-violet-900/30' },
