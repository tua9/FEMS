import { ChevronLeft, ChevronRight, Clock, Laptop, MapPin, User, Users, X } from 'lucide-react';
...
const STATUS_CFG = {
  not_yet:   { label: 'Upcoming',  dot: 'bg-slate-400',   badge: 'bg-slate-100/80 text-slate-500 dark:bg-slate-800 dark:text-slate-400' },
  ongoing:   { label: 'Ongoing',   dot: 'bg-emerald-400', badge: 'bg-emerald-200/80 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300' },
  completed: { label: 'Completed', dot: 'bg-blue-400',    badge: 'bg-blue-100/80 text-blue-600 dark:bg-blue-900/40 dark:text-blue-300' },
};
...
// ─── Derive visual status from time ──────────────────────────────────────────
...
function deriveStatus(ev, slot, isToday) {
  ...
  if (evDate < todayM) return 'completed';
  if (isToday) {
    const cur = getVNMinutes();
    if (cur > slot.endH * 60 + slot.endM) return 'completed';
    if (cur >= slot.startH * 60 + slot.startM) return 'ongoing';
  }
  return 'not_yet';
}
                  ...
                  {hourEvents.map((ev, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] opacity-70 font-semibold flex items-center gap-1">
                          <User className="w-3 h-3 shrink-0" />{ev.lecturer}
                        </p>
                      </div>
...
// ── Slot Grid View (student variant: shows status badges) ─────────────────────
    ...
    <div className="glass-card rounded-[2rem] overflow-hidden p-4 md:p-6">
      ...
      {FIXED_SLOTS.map(slot => {
        ...
        return (
          <div
            key={slot.order}
            className={`grid grid-cols-[88px_repeat(7,1fr)] border-b border-[#1E2B58]/5 dark:border-white/5 last:border-b-0 min-h-[110px] ${
          ...
          >
            ...
            {days.map((date, dayIdx) => {
              ...
              return (
                <div
                  key={dayIdx}
                  className={`border-l border-[#1E2B58]/5 dark:border-white/5 p-1 flex flex-col gap-1 justify-center ${
                    isToday && isActive ? 'bg-emerald-50/40 dark:bg-emerald-900/10' : ''
                  } ${isWeekend ? 'bg-[#1E2B58]/[0.015] dark:bg-white/[0.01]' : ''}`}
                >
                  {cellEvents.map((ev, eIdx) => {
                    const evDate = new Date(ev.year, ev.month, ev.day);
                    const evIsToday = isSameDay(evDate, TODAY);
                    const status = deriveStatus(ev, slot, evIsToday);
                    const sc = STATUS_CFG[status] || STATUS_CFG.not_yet;
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-1.5 rounded-xl text-white transition-all hover:opacity-90 hover:scale-[1.02] active:scale-95 flex flex-col gap-1 ${TYPE_STYLE[ev.type]?.bg || 'bg-[#1E2B58]'} ${
                          isOngoing ? 'ring-2 ring-emerald-400 ring-offset-1' : ''
                        } ${isDone ? 'opacity-55' : ''}`}
                      >
                        <div className="text-[9px] md:text-[10px] font-extrabold truncate leading-tight">{ev.title}</div>
                        <div className="text-[8px] opacity-80 truncate flex items-center gap-0.5">
                          <MapPin className="w-2.5 h-2.5 shrink-0" />
                          <span className="truncate">{ev.location}</span>
                        </div>
                        <div className="text-[8px] opacity-70 truncate flex items-center gap-0.5">
...
// ── Event Modal (student-specific: Lecturer + Borrow Equipment) ───────────────
const EventModal = ({ event, slot, onClose, onBorrow }) => {
  ...
  const status = slot ? deriveStatus(event, slot, isSameDay(evDate, TODAY)) : 'not_yet';
  const sc = STATUS_CFG[status] || STATUS_CFG.not_yet;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div className="glass-card rounded-[2rem] w-full max-w-md shadow-2xl shadow-[#1E2B58]/20 overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className={`${style.bg} px-6 pt-6 pb-5 flex items-start justify-between`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5 mb-0.5">
                <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${style.badgeBg}`}>{style.badge}</span>
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition-colors shrink-0 ml-2">
        ...
        <div className="p-6 space-y-3">
          ...
          {event.students > 0 && (
            <div className="flex items-center gap-3 p-3 glass-card !rounded-xl">
              ...
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-[#1E2B58]/40 dark:text-white/40">Classmates</p>
...
const StudentSchedulePage = () => {
  ...
  const [selectedEvent, setSelectedEvent] = useState(null); // { event, slot }
  ...
  const weekStats = useMemo(() => {
    ...
    const completed = events.filter(e => {
      ...
      const todayM = new Date(); todayM.setHours(0, 0, 0, 0);
      return e.status === 'completed' || evDate < todayM;
    }).length;
  ...
  return (
    <div className="w-full">
      <main className="pt-6 sm:pt-8 pb-10 px-4 sm:px-6 w-full max-w-350 mx-auto flex-1 flex flex-col">
        ...
        {/* ── Layout: main view + sidebar ── */}
        <div className="flex flex-col xl:flex-row gap-6 flex-1">

          {/* Main calendar view */}
          <div className="flex-1 min-w-0">
            {viewMode === 'month' && (
              <MonthView viewDate={viewDate} events={events} onDayClick={handleDayClick} onEventClick={ev => setSelectedEvent({ event: ev, slot: null })} />
            )}
            {viewMode === 'slot' && (
              <SlotGridView viewDate={viewDate} events={events} onEventClick={(ev, slot) => setSelectedEvent({ event: ev, slot })} />
            )}
            {viewMode === 'week' && (
              <WeekView viewDate={viewDate} events={events} onDayClick={handleDayClick} onEventClick={ev => setSelectedEvent({ event: ev, slot: null })} />
            )}
            {viewMode === 'day' && (
              <DayView date={viewDate} events={events} onEventClick={ev => setSelectedEvent({ event: ev, slot: null })} />
            )}
          </div>
          ...
          <aside className="xl:w-64 shrink-0 flex flex-col gap-4">
            ...
            <div className="glass-card rounded-[1.5rem] p-4">
              ...
              <div className="border-t border-[#1E2B58]/8 dark:border-white/10 pt-3 space-y-2">
                {Object.entries(STATUS_CFG).map(([key, s]) => (
                  <div key={key} className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full shrink-0 ${s.dot}`} />
                    <span className="text-xs font-bold text-[#1E2B58]/70 dark:text-white/60 capitalize">{s.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Weekly Summary */}
            <div className="glass-card rounded-[1.5rem] p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-[#1E2B58]/60 dark:text-white/40 mb-3">This View</p>
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-500">Total classes</span>
                  <span className="text-xs font-black text-[#1E2B58] dark:text-white">{weekStats.total}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-500">Completed</span>
                  <span className="text-xs font-black text-blue-500">{weekStats.completed}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-500">Upcoming</span>
                  <span className="text-xs font-black text-amber-500">{weekStats.upcoming}</span>
                </div>
              </div>
            </div>
          </aside>
      ...
      {selectedEvent && (
        <EventModal
          ...
          onBorrow={() => { setSelectedEvent(null); navigate('/student/equipment'); }}
        />
      )}
    </div>
  );
