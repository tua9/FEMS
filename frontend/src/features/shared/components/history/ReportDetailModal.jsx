...
export const ReportDetailModal = ({ item: r, onClose, onReportAgain }) => {
 ...
 {((r.original)?.images?.length > 0 || r.img) && (
 <div className="pt-2 border-t border-[#1E2B58]/10 dark:border-white/10">
 <span className="text-[#1E2B58]/60 dark:text-white/50 font-medium text-sm block mb-2">Evidence Images</span>
 <div className={`grid ${(r.original)?.images?.length > 1 ? 'grid-cols-2' : 'grid-cols-1'} gap-2`}>
 {(r.original)?.images?.length > 0 ? (
 (r.original).images.map((imgUrl, idx) => (
