...
const STATUS_CONFIG = {
  ...
  approved:   { label: 'Đã duyệt',    color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
  processing: { label: 'Đang sửa',    color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' },
  fixed:      { label: 'Hoàn thành',  color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' },
  rejected:   { label: 'Từ chối',     color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' },
  cancelled:  { label: 'Đã huỷ',      color: 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400' },
};
...
const STATUS_FILTERS = ['all', 'pending', 'approved', 'processing', 'fixed', 'rejected'];
...
const RepairDetailModal = ({ report, onClose, onUpdateStatus }) => {
  ...
  const ACTION_LABELS = {
    approved:   { label: 'Duyệt', icon: 'check_circle', cls: 'bg-blue-600 text-white hover:bg-blue-700' },
    rejected:   { label: 'Từ chối', icon: 'cancel', cls: 'bg-red-500 text-white hover:bg-red-600' },
    processing: { label: 'Bắt đầu sửa', icon: 'build', cls: 'bg-purple-600 text-white hover:bg-purple-700' },
    fixed:      { label: 'Đánh dấu hoàn thành', icon: 'task_alt', cls: 'bg-green-600 text-white hover:bg-green-700' },
  };

  const handleAction = async (newStatus) => {
    setLoading(true);
    await onUpdateStatus(report._id, { status: newStatus, cause: cause || undefined, outcome: outcome || undefined, decision_note: note || undefined });
    setLoading(false);
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="dashboard-card rounded-4xl shadow-2xl w-full max-w-md p-0 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="px-6 pt-6 pb-4 flex items-start justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400">{report.code ?? report._id?.slice(-8).toUpperCase()}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          {report.description && (
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Mô tả</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Người báo</p>
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">{report.user_id?.displayName ?? '—'}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Mức độ</p>
              <p className={`text-xs font-bold uppercase ${PRIORITY_COLOR[report.priority] ?? ''}`}>{report.priority}</p>
            </div>
          </div>
          ...
          {(report.status === 'approved' || report.status === 'processing') && (
            <div className="space-y-2">
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Nguyên nhân</label>
              ...
              {report.status === 'processing' && (
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Kết quả sửa</label>
              ...
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Ghi chú</label>
        ...
        {allowed.length > 0 && (
          <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex gap-2 flex-wrap">
            {allowed.map((s) => {
              ...
              if (!ac) return null;
              return (
                <button key={s} disabled={loading} onClick={() => handleAction(s)}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-base font-extrabold text-[#1A2B56] dark:text-white">Thiết bị đang sửa chữa</h3>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700">
                {['Thiết bị', 'Người báo', 'Mức độ', 'Trạng thái', 'Ngày báo', ''].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {filtered.map((r) => {
                ...
                return (
                  <tr key={r._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition cursor-pointer" onClick={() => setSelected(r)}>
                    <td className="py-3 pr-4">
                      <div className="font-semibold text-slate-800 dark:text-slate-200">
                        {r.equipment_id?.name ?? '—'}
                      </div>
                      <div className="text-[10px] text-slate-400">{r.code ?? r._id?.slice(-8).toUpperCase()}</div>
                    </td>
                    <td className="py-3 pr-4 text-xs text-slate-600 dark:text-slate-400">
                      {r.user_id?.displayName ?? '—'}
                    </td>
                    <td className="py-3 pr-4">
                      <span className={`text-[10px] font-bold uppercase ${PRIORITY_COLOR[r.priority] ?? ''}`}>{r.priority}</span>
                    </td>
                    <td className="py-3 pr-4">
